// Hermetic tests for the agent cache — no SDK, no sessions. Exercises the
...
test("creates once and caches the same promise", async () => {
  let calls = 0;
  const cache = createAgentCache({ dispose: noopDispose });
...
test("disposeAll disposes every agent (awaiting resolution) and empties", async () => {
  const disposed = [];
  const cache = createAgentCache({
    dispose: (p) => Promise.resolve(p).then((agent) => disposed.push(agent.id)).catch(() => {}),
  });
  ...
  assert.deepEqual(disposed.sort(), ["a", "b"], "each agent disposed once resolved");
});

test("disposeAll on a still-pending, never-resolving agent does not throw", () => {
  const cache = createAgentCache({ dispose: (p) => Promise.resolve(p).catch(() => {}) });
